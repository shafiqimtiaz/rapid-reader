...
- **Read aloud, in step with the words** — **Aloud** is a mode toggle, not a transport control: it arms the voice without making a sound. Play/Pause drives everything. With Aloud on, the voice becomes the clock — each `word` event moves the reader onto the word being spoken, so the display cannot drift; the wpm ticker is suspended and wpm instead sets the speech rate (`wpm / 180`, with a per-voice calibration slider). Since `chrome.tts` has no pause/resume, Pause stops the voice and Play re-speaks from the start of the current sentence. Seeking is debounced, so a held arrow key scrubs silently. Speech runs in the service worker on offline system voices, so it survives the overlay closing; if the engine sends no word events the ticker takes back over rather than freezing. With no voice installed, the button and the whole options section stay hidden.
